...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    s = sigmoid(a)
    # 1. Значение всегда между 0 и 1
    assert 0.0 <= s <= 1.0

    # 2. Симметрия: 1 - sigmoid(x) == sigmoid(-x)
    assert_close(1.0 - s, sigmoid(-a))

    # 3. В 0 функция равна 0.5
    if abs(a) < 1e-6:
        assert_close(s, 0.5)

    # 4. Функция строго возрастает: s(x) < s(x+delta)
    EPS = 1e-12
    delta = 1e-5
    if abs(a) < 1e2:
        s1 = sigmoid(a)
        s2 = sigmoid(a + delta)
        assert s2 - s1 >= -EPS  # допускаем равенство, но не убывание
        if abs(s2 - s1) > EPS:  # если отличаются — строго монотонно
            assert s2 > s1
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert_close(sum_list(ls1) + sum_list(ls2), sum_list([x + y for x, y in zip(ls1, ls2)]))
